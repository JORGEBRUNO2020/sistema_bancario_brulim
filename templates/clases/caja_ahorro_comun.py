...
class Caja_ahorro_comun(Cuenta):
    ...
    def get_saldo(listar_saldos, conn, usuario_id):
        ...
        listar_saldos.execute(	'select * from usuario us join datos_usuario du on us.id = du.usuario_id join cuenta cu on us.id = cu.usuario_id join tipo_cuenta tc on tc.id = cu.tipo_cuenta_id where us.id =%s',(usuario_id))
